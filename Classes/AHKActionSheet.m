...
@property (nonatomic, strong) UIView *backgroundBehindTableViewOnIpadView;
@end
...
+ (void)initialize
{
    ...
    [appearance setBlurTintColor:[UIColor colorWithWhite:1.0f
                                                   alpha:0.5f]];
    ...
    [appearance setSelectedBackgroundColor:[UIColor colorWithWhite:0.1f
                                                             alpha:0.2f]];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    NSAttributedString *attrTitle = [[NSAttributedString alloc] initWithString:item.title
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    [self dismissAnimated:YES
              ...
              selctedItem:item
               completion:item.didChooseItemHandler];
}
...
- (void)addButtonWithTitle:(NSString *)title image:(UIImage *)image type:(AHKActionSheetButtonType)type handler:(didChooseItemHandler)handler
{
    ...
    item.image = image;
    item.type = type;
    item.didChooseItemHandler = handler;
...
- (void)show
{
    ...
    [self setUpTableView];
    [self setupBackgroundBehindTableView];
...
- (void)dismissAnimated:(BOOL)animated duration:(NSTimeInterval)duration selctedItem:(id)selectedItem completion:(didChooseItemHandler)completionHandler
{
    ...
    void (^tearDownView)(void) = ^(void) {
        ...
        if (completionHandler) {
            completionHandler(self,selectedItem);
        }
    };
...
- (void)setUpNewWindow
{
    AHKActionSheetViewController *actionSheetVC = [[AHKActionSheetViewController alloc] initWithNibName:nil
                                                                                                 bundle:nil];
...
- (void)setUpCancelButton
{
    ...
    [cancelButton setAttributedTitle:attrTitle
...
- (void)setupBackgroundBehindTableView
{
    if ([UIDevice currentDevice].userInterfaceIdiom == UIUserInterfaceIdiomPad) {
        self.backgroundBehindTableViewOnIpadView = [[UIView alloc] initWithFrame:CGRectMake(0, 0, 320, self.bounds.size.height)];
        [self.backgroundBehindTableViewOnIpadView setBackgroundColor:[UIColor colorWithRed:64 / 255
                                                                                     green:66 / 255
                                                                                      blue:66 / 255
                                                                                     alpha:0.1]];
        [self.blurredBackgroundView addSubview:self.backgroundBehindTableViewOnIpadView];
    }
}

- (void)setUpTableView
{
    CGRect statusBarViewRect = [self convertRect:[UIApplication sharedApplication].statusBarFrame
                                        fromView:nil];
    ...
    [self insertSubview:tableView
           aboveSubview:self.blurredBackgroundView];
...
- (void)setUpTableViewHeader
{
    if (self.title) {
        ...
        NSAttributedString *attrText = [[NSAttributedString alloc] initWithString:self.title
